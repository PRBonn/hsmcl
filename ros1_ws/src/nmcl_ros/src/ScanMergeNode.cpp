...
public:
	ScanMergeNode()
	{
		...
		o_scanPub =  nh.advertise<sensor_msgs::PointCloud2>("scan_merged", 1);
		
		laserFrontSub = std::make_shared<message_filters::Subscriber<sensor_msgs::LaserScan>>(nh, scanFrontTopic, 1);
		laserRearSub = std::make_shared<message_filters::Subscriber<sensor_msgs::LaserScan>>(nh, scanRearTopic, 1);
		

		// ApproximateTime takes a queue size as its constructor argument, hence MySyncPolicy(10)
		lidarSync = std::make_shared<message_filters::Synchronizer<LidarSyncPolicy>>(LidarSyncPolicy(1), *laserFrontSub, *laserRearSub);
